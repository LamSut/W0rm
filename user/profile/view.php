...
$sql = "SELECT COUNT(*) AS total_ctf FROM ctf";
$stmt = mysqli_prepare($db, $sql);
if (mysqli_stmt_execute($stmt)) {
  $result = mysqli_stmt_get_result($stmt);
  $row = mysqli_fetch_assoc($result);
  $total_ctf = $row["total_ctf"];
  mysqli_stmt_close($stmt);
} else {
  echo "Error getting total CTFs: " . mysqli_error($db);
}

$sql = "SELECT COUNT(DISTINCT idctf) AS attempted_ctf FROM ctfAttempt WHERE idacc = ?";
$stmt = mysqli_prepare($db, $sql);
if ($stmt) {
  mysqli_stmt_bind_param($stmt, "s", $_SESSION["idacc"]);
  mysqli_stmt_execute($stmt);
  $result = mysqli_stmt_get_result($stmt);
  $row = mysqli_fetch_assoc($result);
  $attempted_ctf = $row["attempted_ctf"];
  mysqli_stmt_close($stmt);
} else {
  echo "Error retrieving attempted CTF count: " . mysqli_error($db);
}
...
<body>
  ...
  <script>
    ...
    const attemptedCTF = <?php echo $attempted_ctf?>;
    const totalCTF = <?php echo $total_ctf ?>;
